...
/**
 * This class provides an example of basic streaming usage. - List streams - Get
 * a stream - Create a stream - Delete a stream - Publish to a stream - Consume
 * from a stream partition, using a cursor - Consume from a stream, using a
 * group cursor
 */
public class StreamsExample {
	...
	public static void main(String[] args) throws Exception {
		...
		final String compartmentId = "ocid1.compartment.oc1..aaaaaaaavxkbsxpb3j7iml4rxe4hbnjzagu6xmcjnbmctg32ptqvldmhx6yq";
		...
		// This will utilize ListStreams() to determine if a stream exists and return
		// it, or create a new one.
		Stream stream = getOrCreateStream(adminClient, compartmentId, exampleStreamName, partitions);
		...
		// Use a cursor for getting messages; each getMessages call will return a
		// next-cursor for iteration.
		...
		String groupCursor = getCursorByGroup(streamClient, streamId, "exampleGroup", "exampleInstance-1");
	...
	private static String getCursorByGroup(StreamClient streamClient, String streamId, String groupName,
			String instanceName) {
		System.out.println(String.format("Creating a cursor for group %s, instance %s.", groupName, instanceName));
	...
	private static String getCursorByPartition(StreamClient streamClient, String streamId, String partition) {
		System.out.println(String.format("Creating a cursor for partition %s.", partition));

		CreateCursorDetails cursorDetails = CreateCursorDetails.builder().partition(partition).type(Type.TrimHorizon)
				.build();

		CreateCursorRequest createCursorRequest = CreateCursorRequest.builder().streamId(streamId)
	...
	private static Stream getOrCreateStream(StreamAdminClient adminClient, String compartmentId, String streamName,
			int partitions) throws Exception {

		ListStreamsRequest listRequest = ListStreamsRequest.builder().compartmentId(compartmentId)
				.lifecycleState(LifecycleState.Active).name(streamName).build();
		...
		System.out.println(String.format("No active stream named %s was found; creating it now.", streamName));
		...
		// Since stream creation is asynchronous; we need to wait for the stream to
		// become active.
		GetStreamRequest streamRequest = GetStreamRequest.builder().streamId(createdStream.getId()).build();
		Stream activeStream = adminClient.getWaiters().forStream(streamRequest, LifecycleState.Active).execute()
				.getStream();
	...
	private static Stream createStream(StreamAdminClient adminClient, String compartmentId, String streamName,
			int partitions) {
		System.out.println(String.format("Creating stream %s with %s partitions", streamName, partitions));

		CreateStreamDetails streamDetails = CreateStreamDetails.builder().compartmentId(compartmentId).name(streamName)
				.partitions(partitions).build();

		CreateStreamRequest createStreamRequest = CreateStreamRequest.builder().createStreamDetails(streamDetails)
				.build();
	...
	private static void publishExampleMessages(StreamClient streamClient, String streamId) {
		...
		for (int i = 0; i < 100; i++) {
			messages.add(PutMessagesDetailsEntry.builder().key(String.format("messageKey-%s", i).getBytes(UTF_8))
					.value(String.format("messageValue-%s", i).getBytes(UTF_8)).build());
		}
		...
		for (PutMessagesResultEntry entry : putResponse.getPutMessagesResult().getEntries()) {
			if (entry.getError().trim().length() == 0) {
	...
	private static void simpleMessageLoop(StreamClient streamClient, String streamId, String initialCursor) {
		...
		for (int i = 0; i < 10; i++) {
			...
			GetMessagesRequest getRequest = GetMessagesRequest.builder().streamId(streamId).cursor(cursor).limit(10)
					.build();
			...
			for (Message message : getResponse.getItems()) {
				System.out.println(String.format("%s: %s", new String(message.getKey(), UTF_8),
